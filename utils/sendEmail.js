...
exports.sendVerifyEmailWithTokenEmail = async (req,res,next,user,token,defaultSender) => {
  await sendEmailTransportMessage(req,res,next,user,token,'verificationEmail','Welcome to TechBudget',{},emailSender(defaultSender));
}

exports.sendForgotPasswordEmail =  async (req, res, next, user, token,defaultSender) => {
  await sendEmailTransportMessage(req,res,next,user,token,'forgotPasswordEmail','Forgot Password',{resettoken: token,email:user.email},emailSender(defaultSender))
  next()
}

exports.sendResendTokenEmail = async (req, res, next, user, token,defaultSender) => {
  await sendEmailTransportMessage(req,res,next,user,token,'resendTokenEmail','Verify Email',{},emailSender(defaultSender))
}
...
const sendEmailTransportMessage = async (
  ...
  const paramUrl = '?'+(Object.keys(params).length ? objectToQueryString(params) :  `token=${token}`)
